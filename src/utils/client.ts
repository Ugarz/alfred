...
export default function createClient(): CustomClient {
  ...
  if (
    process.env.ANNOUNCES_CHANNEL === null ||
    process.env.ANNOUNCES_CHANNEL === undefined
  )
    throw new Error("Need a ANNOUNCES_CHANNEL");
  ...
  client.announcesChannel = client.channels.cache.get(
    process.env.ANNOUNCES_CHANNEL,
  );

  client
    .login(process.env.TOKEN)
    .then(() => {
      console.log("Client logged in with token");
    })
    .catch((error) => {
      console.error(error);
    });
