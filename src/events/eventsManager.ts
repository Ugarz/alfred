...
type UpdateGuildEventOptions = GuildScheduledEventEditOptions<
  GuildScheduledEventStatus,
...
export class QEventManager {
  ...
  async fetchGuildEvents() {
    if (this.guildId === undefined)
      throw new Error(`GuildId not found with ID: ${this.guildId}`);
    ...
    try {
      ...
      if (guild === undefined)
        throw new Error(`Guild not found with guildId: ${this.guildId}`);
  ...
  async createGuildEvent(options: GuildScheduledEventCreateOptions) {
    ...
    try {
      ...
      if (!options.scheduledEndTime && options.scheduledStartTime) {
        ...
        options.scheduledEndTime = new Date(
