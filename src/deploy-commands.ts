...
async function deployCommands(): Promise<object[]> {
  ...
  console.log("Detected command folders:", commandFolders);
  ...
  for (const folder of commandFolders) {
    ...
    const commandFiles: string[] = fs
      ...
      .filter((file: string): boolean => file.endsWith(EXTENSION));
    console.log(`Detected command files in ${folder}:`, commandFiles);
    ...
    for (const file of commandFiles) {
      ...
      try {
        ...
        console.log(
          `Loaded module from ${filePath}. Has default export: ${!!commandModule.default}`,
        );
        if (commandModule.default) {
          console.log(
            `Module default export has 'data': ${"data" in commandModule.default}, 'execute': ${"execute" in commandModule.default}`,
          );
        }
        if (
          ...
          const commandData: SlashCommandBuilder =
            await commandModule.default.data();
          commands.push(commandData.toJSON());
          console.log(`Successfully loaded command: ${commandData.name}`);
        } else {
          console.log(
...
(async (): Promise<void> => {
  try {
    if (!process.env.CLIENT_ID || !process.env.GUILD_ID)
      throw new Error("CLIENT_ID or GUILD_ID not found");
