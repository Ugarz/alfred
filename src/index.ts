...
import { type Collection, Events, type GuildScheduledEvent, type GuildScheduledEventStatus, type Interaction, type CacheType, SlashCommandBuilder } from 'discord.js'
...
client.on(Events.GuildScheduledEventCreate, (event: GuildScheduledEvent<GuildScheduledEventStatus>
) => {
...
(async () => {
	...
	for (const folder of commandFolders) {
		...
		const commandFiles: string[] = readdirSync(commandsPath).filter((file: string): boolean => file.endsWith(EXTENSION))
